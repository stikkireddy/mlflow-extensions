...
try:
    ...
    print("Error importing huggingface_hub module please run pip install huggingface_hub or upgrade the sdk "
...
def ensure_chat_template(tokenizer_file: str, chat_template_key: str = "chat_template"):
    ...
    assert tokenizer_file_path.exists(), f"Tokenizer config file not found at {tokenizer_file}"
    with open(tokenizer_file, "r") as f:
        tokenizer_config = json.loads(f.read())
        chat_template = tokenizer_config.get(chat_template_key)
        if chat_template is None:
            raise ValueError(f"Chat template not found in tokenizer config file {tokenizer_file}")
