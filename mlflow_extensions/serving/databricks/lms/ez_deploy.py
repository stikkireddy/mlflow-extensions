...
from databricks.sdk.service.serving import EndpointCoreConfigInput, ServedEntityInput, EndpointTag

from mlflow_extensions.serving.databricks.lms.gpu_configs import GPUConfig, Cloud, ALL_VALID_GPUS
...
class ServingConfig:
    ...
    def smallest_gpu(self, cloud: Cloud) -> GPUConfig:
        ...
        if self.minimum_memory_in_gb is not None:
            filtered_gpus = [gpu for gpu in filtered_gpus if gpu.total_memory_gb > self.minimum_memory_in_gb]
        if len(filtered_gpus) == 0:
            raise ValueError("No valid gpus in cloud please provide the right gpu config")
...
class EzDeploy:
    ...
    def __init__(
        ...
        assert self._registered_model_name.split(".") == 3, ("Ensure that your registered model name "
    ...
    def download(self):
        self._model = CustomServingEnginePyfuncWrapper(
            engine=self._config.engine_proc,
    ...
    def register(self):
        ...
        assert self._registered_model_name is not None, "Ensure you provide a valid registered_name"
        ...
        with mlflow.start_run():
            logged_model = mlflow.pyfunc.log_model(
                ...
                pip_requirements=self._config.pip_config_override or self._model.get_pip_reqs(),
                signature=infer_signature(model_input=["string-request"], model_output=["string-response"]),
            ...
            mlflow.log_params({
                ...
                "serialized_engine_config": str(json.dumps(asdict(self._config.engine_config)))
            })
            self._latest_registered_model_version = logged_model.registered_model_version
    ...
    def deploy(self, name, *, scale_to_zero: bool = True,
